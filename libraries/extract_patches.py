#=================================================================================================
# The file extract random image patches based on the fov (optional) during training and testing.
# The image patches needs to be spliced after prediction on the patches in the test phase.
#=================================================================================================
...
def load_file_path_txt(file_path):
    '''
    Read the data path index file and load the images from disk with txt file
    '''
    img_list = []
    gt_list = []
    fov_list = []
...
def extract_random(full_imgs,full_masks,full_FOVs, patch_h,patch_w, N_patches, inside='not'):
    ...
    iter_tot = 0   #iteration over the total numbe rof patches (N_patches)
        ...
        while k <patch_per_img:
            ...
            if inside=='center' or inside == 'all':
                if not is_patch_inside_FOV(x_center,y_center,full_FOVs[i,0],patch_h,patch_w,mode=inside):
                    continue #end the current iteration and continues to the next iteration
